return {
    nilIfEmpty = function( this, s )
        if s == "" then
            return nil
        end
        return s
    end,
    WriteScores = function( this )
        local template = {
            Version = 1,
            Theme = THEME:GetThemeDisplayName(),
            ThemeVersion = tostring( GetThemeVersion() or "" ),
            ProductID = ProductID(),
            ProductVersion = ProductVersion(),
            MachineGuid = PROFILEMAN:GetMachineProfile():GetGUID(),
            GameMode = SL.Global.GameMode or "",
        }

        local json = LoadModule("Json.Parse.lua")
        ...
        for player in ivalues(GAMESTATE:GetHumanPlayers()) do
            ...
            if course ~= nil then
                ...
                content.Course = {
                    Path = this:nilIfEmpty(course:GetCourseDir()),
                    FullTitle = fullTitle,
                    TranslitFullTitle = course:GetTranslitFullTitle(),
                    Scripter = this:nilIfEmpty(course:GetScripter()),
                    Description = this:nilIfEmpty(course:GetDescription()),
                }
                content.Trail = {
            ...
            elseif song ~= nil then
                ...
                content.Song = {
                    ...
                    SubTitle = this:nilIfEmpty(subtitle),
                    Artist = this:nilIfEmpty(artist),
                    TranslitTitle = song:GetTranslitMainTitle(),
                    TranslitSubTitle = this:nilIfEmpty(song:GetTranslitSubTitle()),
                    TranslitArtist = this:nilIfEmpty(song:GetTranslitArtist()),
                    Genre = this:nilIfEmpty(song:GetGenre()),
